...
static HBRUSH g_hInputBrush = nullptr;
static std::wstring g_saveDirectory;

constexpr COLORREF kBackgroundColor = RGB(242, 246, 252);
constexpr COLORREF kBackgroundGradientBottom = RGB(230, 239, 252);
constexpr COLORREF kSurfaceColor = RGB(252, 253, 255);
constexpr COLORREF kSurfaceShadowColor = RGB(228, 234, 246);
constexpr COLORREF kSurfaceBorderColor = RGB(214, 224, 240);
constexpr COLORREF kTextColor = RGB(30, 34, 42);
constexpr COLORREF kSubtleTextColor = RGB(96, 108, 124);
...
static void DrawRoundedRectFill(HDC hdc, const RECT& rc, int radius,
...
static void PaintWindowBackground(HDC hdc, const RECT& rc) {
    FillVerticalGradient(hdc, rc, kBackgroundColor, kBackgroundGradientBottom);
}

static void PaintFluentCards(HWND hWnd, HDC hdc) {
    RECT rc{};
    GetClientRect(hWnd, &rc);

    const RECT sectionTop = {12, 10, rc.right - 12, 252};
    const RECT sectionBottom = {12, 262, rc.right - 12, rc.bottom - 12};

    RECT topShadow = sectionTop;
    RECT bottomShadow = sectionBottom;
    OffsetRect(&topShadow, 0, 2);
    OffsetRect(&bottomShadow, 0, 2);

    DrawRoundedRectFill(hdc, topShadow, 16, kSurfaceShadowColor, kSurfaceShadowColor);
    DrawRoundedRectFill(hdc, bottomShadow, 16, kSurfaceShadowColor, kSurfaceShadowColor);
    DrawRoundedRectFill(hdc, sectionTop, 16, kSurfaceColor, kSurfaceBorderColor);
    DrawRoundedRectFill(hdc, sectionBottom, 16, kSurfaceColor, kSurfaceBorderColor);

    HPEN accentPen = CreatePen(PS_SOLID, 2, kPrimaryBtnColor);
    HGDIOBJ oldPen = SelectObject(hdc, accentPen);
    MoveToEx(hdc, sectionBottom.left + 20, sectionBottom.top + 1, nullptr);
    LineTo(hdc, sectionBottom.left + 120, sectionBottom.top + 1);
    SelectObject(hdc, oldPen);
...
static void CreateControls(HWND hWnd) {
    ...
    int y = 24, gap = 42, lblW = 82, left = 26;

    CreateLabel(hWnd, L"下载链接:", left, y + 3, lblW, 22);
    CreateEdit(hWnd, IDC_URL_EDIT, L"", left + lblW, y, 550, 28);

    y += gap;
    CreateLabel(hWnd, L"保存目录:", left, y + 3, lblW, 22);
    CreateEdit(hWnd, IDC_PATH_EDIT, g_saveDirectory.c_str(), left + lblW, y, 468, 28);
    CreateBtn(hWnd, IDC_PATH_BROWSE, L"浏览...", left + lblW + 476, y, 74, 28);

    y += gap;
    CreateLabel(hWnd, L"线程数:", left, y + 3, 55, 22);
    CreateEdit(hWnd, IDC_THREADS_EDIT, L"8", left + 55, y, 54, 28, ES_NUMBER);
    CreateLabel(hWnd, L"限速(KB/s):", left + 128, y + 3, 85, 22);
    CreateEdit(hWnd, IDC_SPEEDLIMIT_EDIT, L"0", left + 216, y, 84, 28, ES_NUMBER);
    CreateLabel(hWnd, L"0 表示不限速", left + 312, y + 3, 100, 22);

    y += gap;
    CreateLabel(hWnd, L"UA伪装:", left, y + 3, 65, 22);
    HWND hCombo = CreateWindowW(L"COMBOBOX", L"",
        WS_CHILD | WS_VISIBLE | CBS_DROPDOWNLIST | WS_VSCROLL,
        left + 65, y, 220, 220, hWnd, (HMENU)(intptr_t)IDC_UA_COMBO,
        g_hInst, nullptr);
    ...
    HWND hCustomUa = CreateEdit(hWnd, IDC_UA_CUSTOM, L"", left + 296, y, 336, 28);
    SendMessageW(hCustomUa, EM_SETCUEBANNER, TRUE,
                 reinterpret_cast<LPARAM>(L"请输入自定义 UA"));
    ShowWindow(hCustomUa, SW_HIDE);
    EnableWindow(hCustomUa, FALSE);

    y += gap + 5;
    HWND hDownBtn = CreateBtn(hWnd, IDC_DOWNLOAD_BTN, L"开始下载", left, y, 632, 40);
    ...
    HWND hProg = CreateWindowW(PROGRESS_CLASSW, L"",
        ...
        left, y, 632, 26, hWnd, (HMENU)(intptr_t)IDC_PROGRESS,
        g_hInst, nullptr);
    SetWindowTheme(hProg, L"Explorer", nullptr);
    SendMessageW(hProg, PBM_SETRANGE, 0, MAKELPARAM(0, 1000));
    SendMessageW(hProg, PBM_SETBARCOLOR, 0, kPrimaryBtnColor);
    SendMessageW(hProg, PBM_SETBKCOLOR, 0, RGB(230, 237, 247));

    y += 36;
    HWND hStatus = CreateWindowW(L"STATIC", L"就绪",
        WS_CHILD | WS_VISIBLE | SS_LEFT,
        left, y, 632, 22, hWnd, (HMENU)(intptr_t)IDC_STATUS_LABEL,
        g_hInst, nullptr);

    y += 28;
    HWND hInfo = CreateWindowW(L"STATIC", L"",
        WS_CHILD | WS_VISIBLE | SS_LEFT,
        left, y, 632, 22, hWnd, (HMENU)(intptr_t)IDC_INFO_LABEL,
        g_hInst, nullptr);
...
static void OnDownloadComplete(HWND hWnd, bool success) {
    ...
    int idx = static_cast<int>(
        SendDlgItemMessageW(hWnd, IDC_UA_COMBO, CB_GETCURSEL, 0, 0));
    HWND hCustomUa = GetDlgItem(hWnd, IDC_UA_CUSTOM);
    const BOOL useCustom = idx == 4;
    EnableWindow(hCustomUa, useCustom);
    ShowWindow(hCustomUa, useCustom ? SW_SHOW : SW_HIDE);
...
static LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_CREATE:
        LoadAppSettings();
        if (!g_hInputBrush) {
            g_hInputBrush = CreateSolidBrush(kInputBackgroundColor);
        }
    ...
    case WM_ERASEBKGND: {
        RECT rc;
        GetClientRect(hWnd, &rc);
        PaintWindowBackground((HDC)wParam, rc);
        return 1;
    }
    ...
    case WM_PAINT: {
        ...
        RECT rc;
        GetClientRect(hWnd, &rc);
        PaintWindowBackground(hdc, rc);
        // 叠加两层圆角卡片，形成 Fluent 风格的分区层次。
        PaintFluentCards(hWnd, hdc);
        EndPaint(hWnd, &ps);
        return 0;
    }
    ...
    case WM_CTLCOLORSTATIC: {
        ...
        const int ctrlId = GetDlgCtrlID(hCtrl);

        if (ctrlId == IDC_UA_CUSTOM) {
            SetBkMode(hdc, OPAQUE);
            SetBkColor(hdc, kInputBackgroundColor);
            SetTextColor(hdc, kSubtleTextColor);
            return reinterpret_cast<LRESULT>(g_hInputBrush);
        }
    ...
    case WM_COMMAND: {
        ...
        case IDC_UA_COMBO: {
            if (HIWORD(wParam) == CBN_SELCHANGE) {
                ...
                if (useCustom) {
                    SetFocus(hCustomUa);
                    SendMessageW(hCustomUa, EM_SETSEL, 0, -1);
                }
            }
            break;
        }
...
int RunDownloaderApp(HINSTANCE hInstance, int nCmdShow) {
    g_hInst = hInstance;
